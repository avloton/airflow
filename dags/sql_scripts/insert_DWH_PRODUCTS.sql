with t as (
    select distinct on (stg.product_id) cast(stg.product_id as integer),
        stg.brand,
        stg.product_line,
        stg.product_class,
        stg.product_size,
...
) merge into DWH_PRODUCTS p using t on t.product_id = p.product_id
when matched then
update
set brand = t.brand,
    ...
    product_first_sold_date = t.product_first_sold_date
